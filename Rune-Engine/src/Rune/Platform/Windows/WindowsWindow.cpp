...
namespace Rune {
	...
	void WindowsWindow::Init(const WindowProps& props) {
		...
		if (s_GLFWWindowCount == 0) {
			RUNE_CORE_INFO("Initializing GLFW");
		...
		glfwSetKeyCallback(m_Window, [] (GLFWwindow* window,int key, int scancode, int action, int mods) { // TODO will need to convert to own key codes at some point
			WindowData& data = *(WindowData*) glfwGetWindowUserPointer(window);
	...
	void WindowsWindow::Shutdown() {
		...
		if (--s_GLFWWindowCount == 0)
		{
			RUNE_CORE_INFO("Terminating GLFW");
			glfwTerminate();
		}
	}
